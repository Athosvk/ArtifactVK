...
void ModelRenderer::RunRenderLoop()
{
...
Texture2D& ModelRenderer::LoadImage()
{
...
DepthAttachment &ModelRenderer::CreateSwapchainDepthAttachment()
{
    return m_VulkanInstance.GetActiveDevice().CreateSwapchainDepthAttachment();
}

UniformConstants ModelRenderer::GetUniforms()
{
...
RasterPipeline ModelRenderer::LoadShaderPipeline(VulkanDevice &vulkanDevice, const RenderPass &renderPass) const
{
...
void ModelRenderer::RecordFrame(PerFrameState& state)
{
...
std::vector<std::reference_wrapper<Semaphore>> ModelRenderer::CreateSemaphorePerInFlightFrame()
{
...
std::vector<PerFrameState> ModelRenderer::CreatePerFrameState(VulkanDevice &vulkanDevice)
{
...
std::vector<Vertex> ModelRenderer::GetVertices() const {
...
std::vector<uint32_t> ModelRenderer::GetIndices() const
{
...
const DescriptorSetLayout& ModelRenderer::BuildDescriptorSetLayout(VulkanDevice &vulkanDevice) const
{
